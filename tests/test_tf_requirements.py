"""
Test TF (Toroidal Field) IDS requirement resolution.

Single parametric test that verifies resolve() can fully resolve all requirements
for each tf field, tracking resolution depth.
"""
import pytest
...
def test_can_resolve_requirements(ids_path, composer):
    """Test that resolve() can fully resolve requirements for each tf field."""
